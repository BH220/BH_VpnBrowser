...
1. **링크로 연 새 탭이 이동하지 않고 기다리던 문제**
   `OnNewWindowRequested` 안에서 새 WebView2 초기화를 `await` 하면 그동안 요청한 페이지가
   deferral 에 묶여 멈춥니다. 예비 탭(`_spareTab`)을 미리 초기화해 두고 즉시 넘겨주도록 바꿨습니다.
   예비 탭이 아직 없으면 기다리지 않고 우리가 직접 새 탭을 엽니다.
   예비 탭은 `_tabs` 에 넣지 않고 `Visibility.Hidden` 으로 둡니다
   (`Collapsed` 면 WPF 가 창을 만들지 않아 초기화가 끝나지 않습니다).

2. **현재 외부 IP 확인이 기존 탭을 갱신하던 문제** → 새 탭에서 엽니다.

3. **확대/축소 표시·기능 추가** (크롬처럼 주소창 오른쪽 끝)
   `ZoomButton` 칩 + 팝업(− / 배율 / + / 재설정), Ctrl+0 / Ctrl+± 단축키,
   `ZoomFactorChanged` 구독으로 Ctrl+휠 확대도 표시에 반영. 배율 단계는 크롬과 동일.
   커스텀 템플릿 버튼이라 UIA 이름이 없어서 `AutomationProperties.Name` 도 넣었습니다.

### 검증 상태

- 확대/축소: 실제 앱에서 확인 완료(칩 표시, 팝업, 150% 까지 단계 확대, 재설정 100%)
- IP 확인 새 탭 / 링크로 새 탭 열기: **아직 육안 확인 못 했습니다**(빌드는 통과).
  사용자가 Visual Studio 로 작업 중이라 화면 자동화를 중단했습니다.
