...
namespace BH_VpnBrowser.Services
{
    ...
    public sealed class Socks5ProxyServer : IDisposable
    {
        ...
        /// <summary>이 앱의 프로세스가 아니어서 거절한 접속 수. 진단용입니다.</summary>
        ...
        private async Task AcceptLoopAsync(CancellationToken token)
        {
            while (!token.IsCancellationRequested)
            {
                ...
                // 인증이 없는 대신, 같은 PC 의 다른 프로그램이 이 터널을 빌려 쓰지 못하게
                // 접속한 프로세스가 이 앱(또는 그 자식인 WebView2 프로세스)인지 확인합니다.
        ...
        private bool IsTrustedPeer(TcpClient client)
        {
            ...
            var pid = LocalPeerIdentifier.FindOwningProcess(peer, Port);
            return pid is { } owner && LocalPeerIdentifier.IsCurrentProcessOrDescendant(owner);
        }
